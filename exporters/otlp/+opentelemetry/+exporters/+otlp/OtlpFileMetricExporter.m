classdef OtlpFileMetricExporter < opentelemetry.sdk.metrics.MetricExporter
% OtlpFileMetricExporter exports metrics in OpenTelemetry Protocol format to
% one or more files. 

% Copyright 2024 The MathWorks, Inc.

    properties
        FileName (1,1) string = "metrics-%N.jsonl"       % Output file name
        AliasName (1,1) string = "metrics-latest.jsonl"  % Alias file name, which is the latest file in rotation     
        FlushInterval (1,1) duration = seconds(30)     % Time interval between metric exports
    ...
    methods
        function obj = OtlpFileMetricExporter(optionnames, optionvalues)
            % OtlpFileMetricExporter exports metrics in OpenTelemetry Protocol 
            % format to one or more files.
            %    EXP = OPENTELEMETRY.EXPORTERS.OTLP.OTLPFILEMETRICEXPORTER
            %    creates an exporter that uses default configurations.
            %
            %    EXP =
            %    OPENTELEMETRY.EXPORTERS.OTLP.OTLPFILEMETRICEXPORTER(PARAM1,
            ...
            %                             name is "metrics-%N.jsonl"
            %       "AliasName"         - Alias file name, which is the 
            %                             latest file in rotation. Can
            %                             contain pattern placeholders.
            %                             Default name is "metrics-latest.jsonl"
            %       "FlushInterval"     - Time interval between metric
            ...
            obj = obj@opentelemetry.sdk.metrics.MetricExporter(...
                "libmexclass.opentelemetry.exporters.OtlpFileMetricExporterProxy");
