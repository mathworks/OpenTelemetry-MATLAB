classdef OtlpFileSpanExporter < opentelemetry.sdk.trace.SpanExporter
% OtlpFileSpanExporter exports spans in OpenTelemetry Protocol format to
% one or more files. 

% Copyright 2024 The MathWorks, Inc.

    properties
        FileName (1,1) string = "trace-%N.jsonl"       % Output file name
        AliasName (1,1) string = "trace-latest.jsonl"  % Alias file name, which is the latest file in rotation     
        FlushInterval (1,1) duration = seconds(30)     % Time interval between span exports
    ...
    methods
        function obj = OtlpFileSpanExporter(optionnames, optionvalues)
            % OtlpFileSpanExporter exports spans in OpenTelemetry Protocol 
            ...
            %
            %    EXP =
            %    OPENTELEMETRY.EXPORTERS.OTLP.OTLPFILESPANEXPORTER(PARAM1,
            ...
            %                             name is "trace-%N.jsonl"
            %       "AliasName"         - Alias file name, which is the 
            %                             latest file in rotation. Can
            %                             contain pattern placeholders.
            %                             Default name is "trace-latest.jsonl"
            %       "FlushInterval"     - Time interval between span
            ...
            obj = obj@opentelemetry.sdk.trace.SpanExporter(...
                "libmexclass.opentelemetry.exporters.OtlpFileSpanExporterProxy");
